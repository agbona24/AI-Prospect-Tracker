...
export default function BusinessCard({ business, onClick }: Props) {
  ...
  const quickWhatsApp = (e: React.MouseEvent) => {
    ...
    // Truncate long names — Google sometimes returns full descriptions in parentheses
    ...
    // Handle generic "Business" category — avoid "business businesses"
    const rawCat = business.category.toLowerCase();
    const isGeneric = rawCat === 'business' || rawCat === 'establishment' || rawCat === 'point of interest';
